...
class MultiLayerBarChart extends D3Compose {
  initialize() {
    ...
    this.configDefine('margin', {
      defaultValue: { top: 20, right: 20, bottom: 30, left: 45 },
    });
    ...
    this.layer('y-axis', yAxisGroup, {
      // Bind tick values from the y-scale — not the chart data
      dataBind: (selection) => {
        return selection.selectAll('.tick').data(this.yScale.ticks(TICK_COUNT));
      },
      insert: (selection) => {
      ...
      events: {
        ...
        'merge:transition': (transition) => {
          ...
          transition.selection().select('text').text((d) => fmt(d));
        },
    ...
    this.layer('x-axis', xAxisGroup, {
      // Bind the category labels extracted from the chart data
      dataBind: (selection, data) => {
        return selection.selectAll('.tick').data(data.map((d) => d.label));
      },
      insert: (selection) => {
        const tick = selection.append('g').attr('class', 'tick');
        tick.append('line');
...
2. **Y-axis derives its own data** — `dataBind` ignores the incoming data array and uses `this.yScale.ticks()` instead. This is valid: `dataBind` receives the chart data but is free to bind any data it wants.
...
4. **Properties set in `initialize()`** — scales and dimensions are assigned in `initialize()`, which the parent constructor calls automatically.
